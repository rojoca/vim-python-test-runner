...
def get_proper_command(desired_command, current_directory):
    ...
    return wrap_command(current_directory, FUNCTIONS[desired_command]())
...
def run_desired_command_for_os(command_to_run, current_directory):
    if "nose" in vim.eval("a:command_to_run") or "nose" in command_to_run:
        ...
        vim.command(":!{0} 2>&1 | {1} | sed 's/\r$//'| tee /tmp/test_results.txt".format(
            command_to_run, get_sed_command(current_directory)))
...
def main():
    ...
    if no_errors(command_to_run):
        run_desired_command_for_os(command_to_run, current_directory)
